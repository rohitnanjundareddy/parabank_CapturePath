...
**Three declarations, not one.** A step says separately what each kind of miss means, because they support different claims:

| declaration | means | is |
|---|---|---|
| `ExtractStep.on_empty` | the results area was found and is empty | an answer |
| `SelectStep.on_value_absent` | the option the caller asked for is not listed | an answer |
| `on_exhausted` | the element could not be found at all | a fault |

Collapsing these is how a capability reports "no transactions" for a table it never found. I shipped that bug twice: once when hardening wrote the empty case into `on_exhausted`, and once when a refusal to read a dropdown fell through to a step's declared outcome and told the caller `ACCOUNT_OPEN_FAILED` for an account that had just been opened. The rule that came out of it is absolute: **a fault is never an answer.** A defect in the artifact is a fact about the artifact, never a fact about the world, and the engine converts any such outcome back into a failure before it can reach a caller.

The recorder fills these in where it can tell. An extract whose sample looked like a collection gets `on_empty` and a declared outcome, because an empty list is a real answer; an extract of a single value does not, because a balance field that exists and is blank is a reading not to be trusted. A select whose value is a caller parameter gets `on_value_absent`, because an option that is not there is a fact about the request.
...
**A ref outlives nothing.** The single worst defect in this build was not in replay at all. Observations stamp a handle onto each element and hand the model a digest keyed on those handles — and every observation numbered them `e0..eN`. A handle the model had seen on a previous page therefore still resolved, to whatever now sat at that number. Extracts meant for a results table were recorded against an account dropdown, a blank date field and a navigation link on three separate runs, each returning something non-empty that parsed cleanly and reported success.

It took me four attempts to find, and the first three were all fixes to the wrong thing: naming the dropdown better in the digest, spelling the target out in the goal, printing what was read at the end of a run. The digest was fine all along — I only learned that by instrumenting what the model actually saw, which I should have done first. Handles now carry the generation that minted them (`o1e18`, `o2e18`) and each observation clears the previous generation's stamps, so a stale handle matches nothing and fails loudly.

The two fixes that held share a shape worth naming: they made a bad recording **impossible** rather than **less likely**. Advice to the model — better names, clearer goals — is advice it can ignore. The driver refusing to read a `<select>` or a blank `<input>` as data cannot be ignored, because reading the options of a picker is never a result and an empty form field means nobody typed in it. Both refusals apply on the discovery path and the replay path, which I got wrong the first time by guarding only one of them.
...
**A handoff is only worth it when a human can change the outcome.** Three situations fail that test and must never raise one, and each cost an operator's time before I separated them. A value the caller supplied that the surface does not have — nobody can make account 99999 exist, and the run already has the answer. A policy refusal — the gate stopped automation deliberately, though an attended operator may still act on their own authority, so this one does hand over by default and can be turned off. And a recording that points at the wrong element — a defect in the artifact, not a state of the page, and no amount of clicking moves the step somewhere else.

Bounds matter as much as the rule. `--max-escalations` caps a discovery run, a reason repeated after an intervention stops it at once, and replay allows one handoff per step. Without those I watched a stuck run ask the same operator the same question three times while nothing changed.
...
**A handoff has to complete the recording, not just the task.** This was a cut and I reversed it. When retries ran out and a person fixed the step by hand, their actions reached evidence and stopped there, so the capability could not replay the part they rescued — verification correctly refused it, with every parameter they had filled in reported frozen. The transfer demo failed that way three times before I changed it.

The reason it had been cut was that turning DOM events into robust locators looked like a large piece of work with a poor robustness story. That turned out to be the wrong read of my own code: `_candidates_for_ref` was already two separable halves — a JS expression that reads an element's properties, and pure Python that turns those properties into a ladder. Splitting them let the handoff listeners call the same inspection on whatever the person clicked and hand the properties back, so a rescued step is targeted no differently from one the agent performed. One description of how to read an element, two callers, no second implementation to drift.

Three things are deliberately not recorded. A password typed during a handoff is dropped outright, because authentication belongs to the login fragment and a secret has no place in a step even as a placeholder. An element the page could not describe is dropped, because a step with no way to find its element again is not a step. And what a person typed is carried as *data* for the recorder to match against a supplied parameter — never used to name the element, for the same reason `value` is not a locator (§2).

What a handoff still cannot do is rewrite an artifact during a replay. Replay never writes to the store, so a replay that needed a person asks again next time; recording a better artifact is discovery's job.
...
- **Writing back to the store from a replay.** A replay that needs a handoff asks for one again next time. Replay never modifies an artifact, and I did not want it to: the store being a one-way boundary (§1) is what makes replay deterministic, and a run that can rewrite its own definition is no longer a run of that definition. Recording a better artifact is discovery's job, and discovery can now do it (§5).
...
1. **Nothing checks that an extract is aimed at the right kind of thing.** The driver now refuses to read a dropdown or a blank input as data, and handles cannot outlive the observation that minted them, so the ways I actually saw a wrong element get recorded now fail loudly. That is narrower than being correct: an extract aimed at a heading, or at the wrong cell of the right table, still records cleanly and replays as a confident wrong answer. Of the seven artifacts shipped, one reads a dropdown and one never produces its declared output. The fix is a validation pass at record time — an extract whose source text equals the page heading, or matches a control's options rather than a value, should fail verification the way a frozen parameter does. Today the only real check is a human reading the `Read from the page:` block, which is why that block exists.
2. **A missed locator can still become a confident wrong answer.** `on_empty` and `on_value_absent` now separate "found and empty" and "your value is not listed" from "could not find it at all", so the common conflation is closed. What remains is a ladder that resolves the *wrong* element and reads something plausible off it — the identity rules narrow that, `review_notes` flags single-rung steps, and neither proves it.
3. **A repeated-extract step type** producing a typed list, replacing the text block above.
4. **A capability's shape is frozen at record time.** Parameters are values inside the recorded flow, never structure, so "update the city" and "update the phone number" are two capabilities unless one is recorded that writes both — and a recording that writes both always writes both. Optional or conditional steps would fix it, and they are also the first thing that would reintroduce non-determinism into replay, which is why the schema does not have them.
5. **`_is_collection` assumes Western currency.** It decides both how a reading is typed and what an empty read means, and its "more than one amount" test keys off `$`, `€` and `£`. The newline and tab half is currency-neutral and does most of the work, but a single-row table of other currencies could be misread as a scalar. I widened its blast radius by reusing it for `on_empty` without widening the test.
6. **Hardening attaches detectors to whatever steps exist**, so a thin recording collects unrelated detectors on its entry navigation. Harmless, since the text never matches, but noise in an artifact meant to be reviewed.
7. **Cross-tenant overlays are designed, not built.** The fragment layer is the substrate; the next step is demonstrating one artifact against two variants of the same app with a single overridden fragment.
8. **Assisted fallback:** a single-step, policy-checked LLM recovery on replay failure, recorded as evidence and never open-ended.
9. **A golden corpus:** saved page snapshots with human-validated expected extractions, replayed as regression tests, so recording quality is measured rather than discovered in production.

**What I would tell the next person working on this.** Every defect that cost me real time was a *recording* problem wearing a browser problem's clothes, and the pattern repeated: the artifact was wrong, replay executed it faithfully, and the result looked like a runtime failure. The fixes that held were the ones that made a bad recording impossible rather than unlikely — a refusal in the driver, a handle that cannot be reused — while the ones that did not hold were all advice to the model. And when something went wrong three times in a row, the useful move was never another fix; it was printing what the model actually saw.
